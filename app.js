...
function exportToExcel() {
  ...
  link.setAttribute("download", `OHT_Scheduler_Report_${new Date().toISOString().slice(0, 10)}.csv`);
...
// EDD Scheduling calculation engine
function runScheduling(isManual = false) {
  ...
  while (pendingJobs.some(j => !j.scheduled)) {
    ...
    // Sort available jobs by EDD (Earliest Due Date)
    ...
    traceLogs.push({
      ...
      sublogs: [
        ...
        `• 待排程隊伍 (已到達)：${availableJobs.map(j => `${j.id}(交期:${j.due})`).join(' → ')}`,
        `• 排程規則：EDD 優先排程交期最早者`
      ]
    });
...
function renderGanttChart(results, machineCount) {
  ...
  for (let m = 0; m < machineCount; m++) {
    ...
    mLabel.setAttribute('fill', 'var(--accent-cyan)');
  ...
  results.forEach(res => {
    ...
    let shadowColor;
    if (res.tardiness > 2) {
      // Critical Delay (> 2) -> Rose
      shadowColor = '#f43f5e';
      rect.setAttribute('fill', 'url(#grad-critical)');
    } else if (res.tardiness > 0) {
      // Warning Delay (<= 2) -> Amber
      shadowColor = '#f59e0b';
      rect.setAttribute('fill', 'url(#grad-warning)');
    } else {
      // On-time -> Cool sleek Cyan/Blue to reduce visual noise
      shadowColor = '#06b6d4';
...
function switchResultTab(tabId) {
  ...
  if (tabId === 'tab-gantt' && globalResults.length > 0) {
    // Small timeout to allow the browser to lay out the tab and give it a valid clientWidth!
    setTimeout(() => {
      renderGanttChart(globalResults, globalMachineCount);
    }, 50);
  }
